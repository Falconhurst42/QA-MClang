...
void ArgReader::invalidError() const {
    MCLError("Invalid command line arguments.");
}
...
void ArgReader::readStatistics() {
    comp->showStatistics = true;
}
...
void ArgReader::readWarning() {
    std::string name = cur();
    if (name == "all")
        comp->warnSetting = Compiler::WARNSETTING_ALL;
    else if (name == "major")
        comp->warnSetting = Compiler::WARNSETTING_MAJOR;
    else if (name == "none")
        comp->warnSetting = Compiler::WARNSETTING_NONE;
    else
        invalidError();
    next();
}
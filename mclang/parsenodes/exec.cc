...
void ExecNode::bytecode(BCManager &man) {
    ...
    if (man.ret.type != Type("const str"))
        MCLError("Exec-statement input is not of type \"const str\".", loc);
    man.write(BCInstr(INSTR_EXEC_CALL, execType + " " + man.ret.value + " if "
    "score 0ret " + man.comp->scoreboardName + " matches 0", fname));

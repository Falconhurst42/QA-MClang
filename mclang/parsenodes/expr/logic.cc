...
void LogicNode::bytecode(BCManager &man) {
    if (type == PNODE_NOT)
        ...
        MCLError("Unexpected error occured when converting logical "
        "bytecode.", loc);
}

void LogicNode::invalidTypeError() const {
    if (type == PNODE_NOT)
        MCLError("Operator \"!\" does not support operand of type \""
    ...
    if (type == PNODE_OR)
        op = "||";
    MCLError("Operator \"" + op + "\" does not support operands of types \""

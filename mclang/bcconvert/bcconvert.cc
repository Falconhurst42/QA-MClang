...
std::vector<std::string> BCConverter::convertInstr(const BCInstr &instr) {
    ...
    if (instr.type == INSTR_CFC)
        return {"execute if score 0ret " + comp->scoreboardName + " matches 0 "
        "run function " + comp->ns + ":" + instr.arg1};
    MCLError("Unexpected error, reading undefined instruction");
    return {};
}
...
std::vector<std::string> BCConverter::convertStackOp(BCInstr instr) const {
    ...
    MCLError("Unexpected error, reading undefined stack operation instruction");
    return {};
}
...
std::vector<std::string> BCConverter::convertComparison(BCInstr instr) const {
    ...
    if (instr.type == INSTR_EQ)
        op = "=";
    if (op == "")
        MCLError("Unexpected error while converting comparison instruction.");
    return {"scoreboard players set 0itmp " + sb + " 0",
...
std::vector<std::string> BCConverter::convertLogical(BCInstr instr) const {
    ...
    MCLError("Unexpected error while converting logical instruction.");
    return {};
}